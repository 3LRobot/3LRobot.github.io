...
*   **Однородность:** На покрытии должны отсутствовать ямы, 
...
![Сканирование территории](/assets/images/BellaBotPro/pudu_map_scan.png){:.image style="display: block; margin: 20px auto"}!!!!!!!!!!!!!!!!1Сделать новый скриншот
...
{: .note}
Учтите, что позиция и ориентация, в которой находится робот в момент создания точки сохраняется для точки.
	
*Точки назначения* - столы, можно добавлять "одним нажатием" используя **[Добавить пункт назначения]** , а так же нажимая на кнопку со значком паузы сверху робота.
...
![Сохраниние карты](/assets/images/BellaBotPro/pudu_map_saving.png){:.image style="display: block; margin: 20px auto"}
![Загрузка в облако PUDU](/assets/images/BellaBotPro/pudu_map_cloudupload.png){:.image style="display: block; margin: 20px auto"}
...
Нажмите **[Выполнено]**, чтобы сохранить изменения или **[Отменить изменения]**, для отклонения, например если было удалено лишнее.
...
Используйте иструмент **[Площади]** чтобы нарисовать виртуальные стены, зоны с риском падения, особые зоны ограничения RGBD, а так же установите ограничения скорости там, где это необходимо
![Инструмент для виртуальных стен и разметки опасных зон](/assets/images/BellaBotPro/pudu_map_zones.png){:.image style="display: block; margin: 20px auto"}!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
![Редактирование карт в меню](/assets/images/BellaBotPro/pudu_menu_mapedit.png)
...
По умолчанию у робота время и часовой пояс синхронизируются по сети, а изначальная локация передается при привязке робота с указанием локации, где робот планируется к использованию.
...
![Функции робота в меню](/assets/images/BellaBotPro/pudu_func_menu.png){:.image style="display: block; margin: 20px auto"}
3. Внутри раздела так же перейти в **[Функции робота]**, где будет всего одна неактивная функция, которая отвечает за возможность принимать задания с облака PUDU
...
2. В правом верхнем углу окна можно открыть общие настройки режима круиза нажав на знак *Шестеренку*, вернуться к точке ожидания нажав на знак *Вернуться*.\ Чтобы добавить новый сгенерированный маршрут по точкам доставки нажмите на знак *Плюс*
...
![Конфигурация доставки](/assets/images/BellaBotPro/pudu_delivery_configure.png){:.image style="display: block; margin: 20px auto"}(Добавить Картинку с обведенным ОПЦИИ доставка)
...
Так же активируются особенности доставка такие как:\
...
![Конфигурация сбора](/assets/images/BellaBotPro/pudu_recycle_configure.png){:.image style="display: block; margin: 20px auto"}(Добавить Картинку с обведенным ОПЦИИ Утилизация)
Устанавливается поведение на точках сбора и утилизации, скорость движения, а так же голосовые функции для этого режима.

### Сопровождение

**Сопровождение и приветсвие** - это режим, который подразумевает встречу, привлечение и приветсвие гостей и посетителей. Робот стоит на заданной точке, откуда также может помочь в рассадке или сопровождении посетителей до точки. Требует обязательного добавления специальной точки на карте: *Точка приветсвия*
