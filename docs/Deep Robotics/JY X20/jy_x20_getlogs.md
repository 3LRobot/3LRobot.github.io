...
Робот во время работы создает **3 типа** логов:
...
- **Журнал операций** - после нажатия кнопки `Сохранить данные` в приложении, данные о движении, записанные на частоте 200 Гц (каждые 0,005 секунд) за последние 5 минут. Каждый файл можно посмотреть в Microsoft Excel. Расширение файла -  **.csv** и он объединен в один файл с журналом ошибок. Точный путь указан в конце журнала событий.
...
{: .note }
Журнал событий сохраняется в течение 7 дней, просроченные файлы будут удаляться каждый раз, когда при запуске будет недостаточно места на жестком диске.

{: .note}
Пожалуйста, убедитесь, что робот лежит, когда вы нажимаете кнопку `Сохранить данные`.
...
![Пульт управления](/assets/images/deepLogs.png){: width="700px" style="display: block; margin: 0 auto"}
